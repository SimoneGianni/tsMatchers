...
describe("Array >", () => {
    ...
    it("Should match equal arrays delegating matchers", () => {
        check([{ a: 1 }, { b: 2, c: 3 }], [{ a: 1 }, { b: 2, c: 3 }]);
        check([{ a: 1 }, { b: 2, c: 3 }], [{ a: 1 }, is.object.matching({ b: 2 })]);
    });
    ...
    it("arrayMatching negatives", () => {
        checkMessage([1, 2, 3, 4, 5], arrayMatching([7, 8, 9]), /an array with 3.*but no element.*something equal to.*7.*but was \[.*/);
        checkMessage([1, 2, 3, 4, 5], arrayMatching([2, 3, 5]), /items 1 to 2 matched but element 3 expecting something equal to number 5 but was 4,.*but was \[.*/);
        checkMessage([1, 2, 3, 4, 5], arrayMatching([4, 5, 6]), /items 3 to 4 matched but array finished before matching something equal to number 6.*but was \[.*/);
    });
